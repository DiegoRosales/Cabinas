...
void ejecutarRutina(int rutina/*, int execType*/) {
  ...
  mySerial.println();
  //wtv020sd16p.reset();
  delay(300);  //tiempo de encendido del led
  switch (rutina) {
    case '1':
  ...
  // Definicion en el Buffer de las secuencia 
...
void apagar() {
  ...
  shiftReg.digitalWriteMS(1, 5, HIGH);  // Enciende luz verde grupo 3
  delay(9000);  //Rerardo para permitir el termino de la reproduccion de MP3 despues del apagado de los LED
  wtv020sd16p.stopVoice(); //Para detener reproducción de MP3
  shiftReg.digitalWriteMS(1, 5, LOW); // Apaga luz verde grupo 3
  mySerial.flush();
  rutina = 0;
  ROJO1 = false;
  ROJO = 0;
...
void leerbit(uint8_t rutina) {
  ...
  for (int i = 0; i < 16; i = i + 2) {
    ...
    //mySerial.println();
      ...
      mySerial.println(i);
      mySerial.println();*/
    for (int j = 0; j < numPasos; j++) {  // numero de pasos ajustado para duracion de terapia, necesario ajustar si se modifica el tiempo de parpadeo
